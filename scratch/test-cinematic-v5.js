...
const outImg = path.join(rootDir, "scratch", "test_cinematic_v5.jpg");
...
lines.forEach((line, i) => {
  ...
  // Solid base text with 3D shadow and thin rim
  baseTextDrawtexts.push(
    `drawtext=fontfile='${fFont}':text='${esc}':fontsize=${fontSize}:fontcolor=${baseColor}:borderw=2:bordercolor=black@0.6:shadowcolor=black@1.0:shadowx=5:shadowy=8:x=(w-text_w)/2:y=${curY}`
  );
...
const filterGraph = [
  // 1. Graded background with film grain & vignette
  ...
  // 2. Smooth mathematical black gradient scrim (reaching solid black at Y=390)
  `color=c=black:s=1280x720,format=rgba,geq=r=0:g=0:b=0:a='if(lt(Y,120),130*pow((120-Y)/120,2),if(lt(Y,150),0,if(gt(Y,390),255,255*pow((Y-150)/(390-150),1.4))))'[scrim]`,

  // 3. Composite background + scrim + embers
  `[graded_bg][scrim]overlay=0:0[bg_scrim]`,
  `[2:v]scale=1280:720[embers]`,
  `[bg_scrim][embers]overlay=0:0[bg_particles]`,

  // 4. Draw solid base text with shadow onto background
  `[bg_particles]${baseTextDrawtexts.join(",")}[bg_with_base]`,

  // 5. Foil Color Gradings (bright, vibrant, specular highlights)
  // Amber Foil (luminous golden amber)
  ...
  // Radiant Bright Gold Foil (brilliant glowing yellow-gold)
