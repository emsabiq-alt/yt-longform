...
async function generateWithPollinations(promptText, outFile, model = "flux") {
  const prompt = encodeURIComponent(promptText);
  const url = `https://image.pollinations.ai/prompt/${prompt}?model=${model}&width=1280&height=720&nologo=true&key=${key}&seed=${Math.floor(Math.random() * 100000)}`;
  console.log(`[Pollinations ${model}] Generating...`);
  ...
  if (res.ok) {
    ...
    console.log(`[OK] Saved ${outFile} (${buf.length} bytes) in ${duration}s`);
    return true;
  } else {
...
async function main() {
  const promptToba = `hyper-detailed viral YouTube documentary investigation thumbnail in 16:9 widescreen format, matching the exact visual complexity, composition, and high-budget production quality.
COMPOSITE SCENE: Dramatic aerial perspective of Lake Toba caldera volcano with stormy sunset clouds and glowing turquoise water. The bottom half cuts open into a photorealistic subterranean earth cross-section, revealing glowing molten magma fissures, boiling red-orange lava rivers beneath the cliff, with volcanic steam vents rising.
...
Cinematic teal and orange color grade, volumetric lighting, incandescent magma glow, floating embers, 8k resolution.`;
  ...
  await generateWithPollinations(promptToba, "scratch/toba-pollinations-flux.jpg", "flux");
}
